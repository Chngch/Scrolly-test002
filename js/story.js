...
(() => {
  ...
  for (let index = 0; index < cardRects.length - 1; index += 1) {
    ...
    if (progress < 1) {
      return {
        ...
        progress,
        ...(progress === 0
          ? { phase: "hold", checkpointIndex: index }
          : { phase: "motion" }),
        ...transitionMetadata
      };
    }
  ...
  const resolvePanZoomVideoCue = function resolvePanZoomVideoCue(source = {}, useMobile = false) {
  ...
  const url = useMobileVideo ? source.mobileSrc : source.desktopSrc;
  const timeSeconds = useMobileVideo
    ? source.mobileTimeSeconds
  ...
  const useMobile = Boolean(
    frameWindow?.matchMedia?.("(max-width: 700px)").matches
  );
  const visibleLayer =
  ...
  const activeCue = resolvePanZoomVideoCue(mediaSources[activeIndex], useMobile);
  ...
  const fromCue = resolvePanZoomVideoCue(mediaSources[fromIndex], useMobile);
  const toCue = resolvePanZoomVideoCue(mediaSources[toIndex], useMobile);
  ...
  host.querySelectorAll("[id]").forEach((group) => {
    if (!Object.prototype.hasOwnProperty.call(states, group.id)) return;
  ...
  const setSource = (layer, source = {}, camera) => {
    ...
    if (empty) empty.hidden = hasDesktop || hasDesktopVideo || hasDesktopSvg;
    if (desktop) {
      if (hasDesktop) {
    ...
    if (video) {
      ...
      video.hidden = !hasDesktopVideo;
      video.muted = true;
      video.playsInline = true;
      if (hasDesktopVideo) {
        const selectedVideoSrc =
          useMobile && source.mobileKind === "video"
            ? source.mobileSrc
            : source.desktopSrc;
  ...
  if (section?.dataset?.annotationProjectionFrozen === "true") return;
  ...
  const visibleLayer =
    ...
    media.querySelector("[data-pan-zoom-camera-layer]");
  const visual =
    visibleLayer?.querySelector?.("[data-pan-zoom-video]:not([hidden])") ||
    visibleLayer?.querySelector?.("[data-pan-zoom-desktop]");
  const mediaWidth =
    Number(visual?.naturalWidth) || Number(visual?.videoWidth) || 0;
  const mediaHeight =
    Number(visual?.naturalHeight) || Number(visual?.videoHeight) || 0;
  ...
  Array.from(group.querySelectorAll("[data-visual-annotation]")).forEach(
    (annotation) => {
      const pointsDatasetKey = useMobile ? "mobilePoints" : "desktopPoints";
      const pathDatasetKey = useMobile ? "mobilePath" : "desktopPath";
      const coordinateDatasetKey = useMobile
        ? "mobileCoordinateSpace"
        : "desktopCoordinateSpace";
      let sourcePoints = parsePath(annotation.dataset[pathDatasetKey]);
      if (sourcePoints.length < 3) {
        sourcePoints = parsePoints(annotation.dataset[pointsDatasetKey]);
      }
      if (sourcePoints.length < 3) return;
      const authoredCamera = parseCamera(
        useMobile
          ? annotation.dataset.mobileCamera
          : annotation.dataset.desktopCamera
      );
      ...
      const coordinateSpace = ["image", "viewport"].includes(
        annotation.dataset[coordinateDatasetKey]
      )
        ? annotation.dataset[coordinateDatasetKey]
        : annotation.dataset.coordinateSpace;
      if (coordinateSpace !== "image") {
        sourcePoints = sourcePoints.map((point) =>
          unprojectVisualAnnotationPoint(point, authoredProjection)
        );
        annotation.dataset[pointsDatasetKey] = pointsValueFor(sourcePoints);
        annotation.dataset[pathDatasetKey] = JSON.stringify(sourcePoints);
        annotation.dataset[coordinateDatasetKey] = "image";
      }
  ...
  const backgroundVideos = Array.from(
    document.querySelectorAll(".story-background-video")
  );
  const syncBackgroundVideoPlayback = () => {
  ...
  reducedMotion.addEventListener?.("change", syncBackgroundVideoPlayback);
  ...
  panZoomSections.forEach((section) => {
    section.querySelectorAll("[data-pan-zoom-step]").forEach((step) => {
      [
        [step.dataset.desktopSrc, step.dataset.desktopKind],
        [step.dataset.mobileSrc, step.dataset.mobileKind]
      ]
  ...
  const update = () => {
    ...
    scrollSequenceSections.forEach((section) => {
      ...
      section.querySelectorAll("[data-scroll-sequence-clip]").forEach((clip) => {
        ...
        clip.style.setProperty("--sequence-card-width", widthPercent + "%");
        clip.style.setProperty(
    ...
    panZoomSections.forEach((section) => {
      const state = resolveActiveStep(section, "[data-pan-zoom-step]");
      if (!state) return;
      ...
      const mediaSources = state.steps.map((step) => {
        ...
        return {
          ...
          desktopRenditions,
          mobileRenditions,
          hasAnnotations: step.dataset.hasAnnotations === "true",
